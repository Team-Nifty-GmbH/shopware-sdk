...
/**
 * searchMeasurementDisplayUnit
 *
 * Available since: 6.7.1.0
 */
class SearchMeasurementDisplayUnit extends Request implements HasBody
{
	use HasJsonBody;
	...
	{
		return "/search/measurement-display-unit";
	}
    ...
    {
        return array_map(
            fn (array $item) => MeasurementDisplayUnit::from($item),
            $response->json('data') ?? []
        );
    }
}
