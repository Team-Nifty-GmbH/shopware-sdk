...
/**
 * searchProductSearchConfig
 *
 * Available since: 6.3.5.0
 */
class SearchProductSearchConfig extends Request implements HasBody
{
	use HasJsonBody;
	...
	{
		return "/search/product-search-config";
	}
    ...
    {
        return array_map(
            fn (array $item) => ProductSearchConfig::from($item),
            $response->json('data') ?? []
        );
    }
}
