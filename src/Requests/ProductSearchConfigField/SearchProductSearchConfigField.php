 ...
 */
class SearchProductSearchConfigField extends Request implements HasBody
{
    use HasJsonBody;
    ...
    {
        return '/search/product-search-config-field';
    }
    ...
    {
        return array_map(
            fn (array $item) => ProductSearchConfigField::from($item),
            $response->json('data') ?? []
        );
    }
}
