...
/**
 * searchAppShippingMethod
 *
 * Available since: 6.5.7.0
 */
class SearchAppShippingMethod extends Request implements HasBody
{
	use HasJsonBody;
	...
	{
		return "/search/app-shipping-method";
	}
    ...
    {
        return array_map(
            fn (array $item) => AppShippingMethod::from($item),
            $response->json('data') ?? []
        );
    }
}
