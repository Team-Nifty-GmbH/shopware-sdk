...
/**
 * searchMeasurementSystem
 *
 * Available since: 6.7.1.0
 */
class SearchMeasurementSystem extends Request implements HasBody
{
	use HasJsonBody;
	...
	{
		return "/search/measurement-system";
	}
    ...
    {
        return array_map(
            fn (array $item) => MeasurementSystem::from($item),
            $response->json('data') ?? []
        );
    }
}
