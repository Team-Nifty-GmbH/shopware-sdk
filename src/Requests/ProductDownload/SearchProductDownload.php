...
/**
 * searchProductDownload
 *
 * Available since: 6.4.19.0
 */
class SearchProductDownload extends Request implements HasBody
{
	use HasJsonBody;
	...
	{
		return "/search/product-download";
	}
    ...
    {
        return array_map(
            fn (array $item) => ProductDownload::from($item),
            $response->json('data') ?? []
        );
    }
}
