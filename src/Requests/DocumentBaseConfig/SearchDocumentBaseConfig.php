...
/**
 * searchDocumentBaseConfig
 *
 * Available since: 6.0.0.0
 */
class SearchDocumentBaseConfig extends Request implements HasBody
{
	use HasJsonBody;
	...
	{
		return "/search/document-base-config";
	}
    ...
    {
        return array_map(
            fn (array $item) => DocumentBaseConfig::from($item),
            $response->json('data') ?? []
        );
    }
}
