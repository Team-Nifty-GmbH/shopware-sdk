 ...
 */
class SearchAppAdministrationSnippet extends Request implements HasBody
{
	use HasJsonBody;
	...
	{
		return "/search/app-administration-snippet";
	}
    ...
    {
        return array_map(
            fn (array $item) => AppAdministrationSnippet::from($item),
            $response->json('data') ?? []
        );
    }
}
